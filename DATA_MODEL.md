...
| `itemType` | 과제, 시험, 팀 프로젝트, 발표, 퀴즈, 수업 일정, 기타 중 하나. 마감·제출은 과제 계열 이벤트의 날짜·제출 정보로 보존하고 공지는 내용에 맞는 유형 또는 기타로 분류한다. |
...
| `dateCertainty` | 시간 정확도: `exact-date`, `academic-week`, `unknown` |
| `revision` | 같은 이벤트가 새 자료로 보완된 누적 버전 |
| `updateNoticeStatus` | 새 정보 확인 상태: `unread`, `reviewed` |
...
확정 판정은 이벤트명과 과목명을 기본으로 하며, 과제 계열은 정확한 날짜, 요구사항, 작업량 근거, 제출 방식이 필요하고 시험·퀴즈는 정확한 날짜와 시험 범위가 필요하다. 예정 주차만 확인된 시험은 주차를 보존하되 미확정으로 유지한다. 날짜 정확도는 이 판정과 독립적이다.
...
일정 화면은 정보 충분성과 시간 정확도를 분리한다. `date`가 있으면 확정 여부와 무관하게 해당 날짜 한 곳에 표시하고, 미확정이면 상태를 함께 표시한다. `date` 없이 `scheduledWeek`와 신뢰할 수 있는 1주차 시작일이 있으면 같은 이벤트를 복제하지 않은 채 Month에서는 하나의 연속 범위 바로, Today에서는 해당 7일의 미확정 일정으로 투영한다. 날짜와 주차 기준이 모두 없으면 임의 배치하지 않는다.
