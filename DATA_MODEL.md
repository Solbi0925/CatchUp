...
├─ CalendarEvent(Google Calendar 개인 일정)
...
- `PlanAdjustment`는 같은 주에 AI Mate 계획 조정을 몇 번 요청했는지 확인하는 데 사용한다.
...
| `supportedFileFormat` | 이번 MVP에서 선택한 지원 파일 형식 |
...
### 3.4 CalendarEvent - Google Calendar 개인 일정

**쉬운 설명**

Google Calendar에서 가져온 개인 일정이다. Mock 단계에서는 실제 연동 대신 가짜 개인 일정을 사용한다.
...
| `source` | 일정 출처. MVP에서는 Google Calendar만 사용 |
...
사용자가 AI Mate에게 "오늘 할 일을 줄여줘"처럼 계획 변경을 요청한 기록이다. 같은 주에는 최대 3번까지만 가능하다.
...
| `usedCountThisWeek` | 이번 주에 사용한 조정 횟수 |
| `remainingCountThisWeek` | 이번 주에 남은 조정 횟수 |
...
| Google Calendar 개인 일정 | `CalendarEvent` |
...
- AI Mate를 통한 계획 조정은 한 주에 최대 3회다.
- 사용자는 Today의 할 일이나 주간 계획 항목의 내용, 날짜, 시간, 우선순위를 직접 수정하지 않는다. 변경은 AI Mate 요청으로만 한다.
- 외부 일정은 Google Calendar만 사용한다.
- Mock 데이터와 예시에는 실제 개인정보, 실제 일정, API 키, OAuth 토큰을 넣지 않는다.
- 초기 MVP에서 업로드 파일은 PDF 또는 이미지 중 팀이 선택한 한 가지 형식만 지원한다.

## 6. 추후 실제 DB 연결 전에 추가로 결정할 사항

- 최초 MVP에서 지원할 업로드 파일 형식을 PDF와 이미지 중 무엇으로 할지
- 로그인 방식을 무엇으로 할지
- Google Calendar 연결을 나중에 한 사용자가 계획 생성을 요청할 때의 정확한 안내 방식
