...
function formatDuration(duration) {
  if (!duration) return '시간 미정';
  if (/H$/i.test(duration)) return duration;
  const hours = Number(duration.match(/(\d+(?:\.\d+)?)시간/)?.[1] || 0);
  const minutes = Number(duration.match(/(\d+)분/)?.[1] || 0);
  const totalHours = hours + minutes / 60;
  if (!totalHours) return duration;
  return `${Number(totalHours.toFixed(1))}H`;
}

function TaskCard({ plan, onToggle }) {
  const dueDate = plan.deadline || plan.dueDate || plan.deadlineDate || plan.date;
  const dueLabel = dueDate ? `${dueDate.slice(5, 7).replace(/^0/, '')}/${dueDate.slice(8, 10).replace(/^0/, '')} 마감` : '마감일 미정';
  return <View style={[styles.taskCard, plan.completed && styles.taskCardCompleted]}><Pressable onPress={onToggle} style={[styles.taskCheck, plan.completed && styles.taskCheckCompleted]}>{plan.completed ? <Ionicons name="checkmark" size={13} color={COLORS.white} /> : null}</Pressable><View style={styles.taskCopy}><Text numberOfLines={1} style={[styles.taskTitle, plan.completed && styles.taskTitleCompleted]}>{plan.title}</Text><View style={styles.taskMetaRow}><View style={styles.taskDuration}><Ionicons name="time-outline" size={11} color={COLORS.purple} /><Text style={styles.taskDurationText}>{formatDuration(plan.duration)}</Text></View><View style={styles.taskDue}><Ionicons name="calendar-outline" size={10} color={COLORS.muted} /><Text style={styles.taskDueText}>{dueLabel}</Text></View></View></View></View>;
}
...
function DateStrip({ selectedDate, onSelect }) {
  ...
  return <View style={styles.dateStrip}>{dates.map((date) => { const active = date === selectedDate; const parsed = new Date(`${date}T12:00:00`); return <Pressable key={date} onPress={() => onSelect(date)} style={[styles.dateItem, active && styles.dateItemActive]}><Text style={[styles.dateDay, active && styles.dateDayActive]}>{parsed.toLocaleDateString('ko-KR', { weekday: 'short' })}</Text><Text style={[styles.dateNumber, active && styles.dateNumberActive]}>{parsed.getDate()}</Text></Pressable>; })}</View>;
}
...
function MainApp({ calendarEvents, calendarStatus, planItems, onTogglePlan, onSavePlan, onDeletePlan, onSaveEvent, onDeleteEvent }) {
  ...
  const screen = tab === 'Today' ? <TodayScreen onMate={() => setMateVisible(true)} calendarEvents={calendarEvents} planItems={planItems} onTogglePlan={onTogglePlan} onAddEvent={openAddEvent} onEditEvent={openEditEvent} /> : tab === 'Month' ? <MonthScreen onMate={() => setMateVisible(true)} calendarEvents={calendarEvents} planItems={planItems} onAddEvent={openAddEvent} onEditEvent={openEditEvent} /> : <UploadScreen onMate={() => setMateVisible(true)} calendarStatus={calendarStatus} />;
...
export default function App() {
  ...
  const togglePlan = (id) => setPlanItems((items) => items.map((item) => item.id === id ? { ...item, completed: !item.completed, isCompleted: !item.completed } : item));
  ...
  return <><StatusBar barStyle="dark-content" />{stage === 'onboarding' ? <Onboarding onContinue={() => setStage('connect')} /> : stage === 'connect' ? <ConnectScreen onConnected={completeConnect} onSkip={completeConnect} /> : <MainApp calendarEvents={calendarEvents} calendarStatus={calendarStatus} planItems={planItems} onTogglePlan={togglePlan} onSavePlan={savePlan} onDeletePlan={deletePlan} onSaveEvent={saveEvent} onDeleteEvent={deleteEvent} />}</>;
}

const styles = StyleSheet.create({
  appRoot: { flex: 1, width: '100%', maxWidth: 390, minHeight: Platform.OS === 'web' ? '100vh' : undefined, alignSelf: 'center', backgroundColor: COLORS.canvas },
  ...
  countBadge: { color: COLORS.purple, backgroundColor: COLORS.purpleSoft, borderRadius: 9, paddingHorizontal: 7, paddingVertical: 3, fontSize: 10, fontWeight: '900', marginLeft: 8 },
  ...
  taskList: { gap: 10, marginBottom: 25 },
  taskCard: { minHeight: 76, borderRadius: 16, backgroundColor: COLORS.white, paddingVertical: 14, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', shadowColor: '#6F4BE9', shadowOpacity: 0.045, shadowRadius: 10, shadowOffset: { width: 0, height: 3 } },
  taskCardCompleted: { backgroundColor: '#F8F6FC' },
  taskCheck: { width: 19, height: 19, borderRadius: 5, borderWidth: 1, borderColor: '#BDB7CB', marginRight: 12, alignItems: 'center', justifyContent: 'center' },
  taskCheckCompleted: { backgroundColor: COLORS.purple, borderColor: COLORS.purple },
  taskTitleCompleted: { color: COLORS.muted, textDecorationLine: 'line-through' },
  taskCopy: { flex: 1 },
  taskTitle: { color: COLORS.ink, fontSize: 14, lineHeight: 19, fontWeight: '800' },
  taskMetaRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 9 },
  taskDuration: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  taskDurationText: { color: COLORS.purple, fontSize: 11, lineHeight: 14, fontWeight: '900' },
  taskDue: { backgroundColor: '#F2F0F6', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, flexDirection: 'row', alignItems: 'center', gap: 4 },
  taskDueText: { color: COLORS.muted, fontSize: 10, lineHeight: 13, fontWeight: '800' },
  eventList: { gap: 8 },
