...
export function classifyAiMateIntent(rawMessage: string): AiMateIntent {
  ...
  if (/(주간계획).*(업데이트)|업데이트.*(계획)/.test(message)) return "update-plan";
  if (/(도움|무엇을 할 수|사용법|어떻게 써)/.test(message)) return "help";
  if (/주간계획\s*(?:을\s*)?(?:생성|짜)|(?:이번 주|7일)\s*계획.*(?:생성|짜)/.test(message)) return "generate-plan";
  ...
  if (/(현재|기존)\s*주간계획|주간계획.*(수정|조정)|(?:수정|조정|옮겨|줄여|늘려)\s*(?:줘|해)/.test(message)) return "adjust-plan";
